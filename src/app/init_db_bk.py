...
# Data path
DATA_URI = os.path.join(os.path.dirname(__file__), '../../data/Crime_Data_2010_2017.csv')
...
def main():
    ...
    crime_df['date'] = crime_df.apply(lambda x: convert_date(x['date_occurred']), axis=1)
    crime_df['time'] = crime_df.apply(lambda x: convert_time(x['time_occurred']), axis=1)
    crime_df['day_of_week'] = crime_df.apply(lambda x: convert_day_of_week(x['date']), axis=1)
    crime_df = crime_df.drop(columns=['date_occurred', 'time_occurred'])
